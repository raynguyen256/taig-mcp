...
export class AuthManager {
    ...
    async login(username, password) {
        ...
        const res = await axios.post(`${this.config.baseUrl}/auth`, { username: user, password: pass, type: 'normal' });
    ...
    getToken() {
        if (!this.token)
            throw new Error('Not authenticated. Call taiga_login first.');
        return this.token;
    }
    getStatus() {
    ...
    async tryRefresh(refreshToken) {
        try {
            const res = await axios.post(`${this.config.baseUrl}/auth/refresh`, { refresh: refreshToken });
