...
export function registerExportImportTools(server, client) {
    server.tool('taiga_export_project', 'Export an entire Taiga project to a JSON dump file on disk. Useful for backups.', {
    ...
    server.tool('taiga_import_project', 'Import a Taiga project from a JSON dump file on disk. Creates a new project.', {
        ...
        const result = await client.postMultipart('/importer/load_dump', form);
        return {
