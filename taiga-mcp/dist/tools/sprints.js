...
export function registerSprintTools(server, client, _cache) {
    server.tool('taiga_list_sprints', 'List all sprints (milestones) for a project. Optionally filter by open/closed status.', {
        ...
        return client.getAll('/milestones', { project: project_id, closed });
    }));
    server.tool('taiga_get_sprint', 'Get details for a specific sprint including its user stories.', {
        sprint_id: z.number().describe('Sprint (milestone) ID'),
    }, wrapTool(async ({ sprint_id }) => {
        return client.get(`/milestones/${sprint_id}`);
    }));
    server.tool('taiga_get_sprint_stats', 'Get burndown chart data and completion statistics for a sprint.', {
        sprint_id: z.number().describe('Sprint (milestone) ID'),
    }, wrapTool(async ({ sprint_id }) => {
        return client.get(`/milestones/${sprint_id}/stats`);
    }));
    server.tool('taiga_create_sprint', 'Create a new sprint (milestone) for a project.', {
        project_id: z.number().describe('Project ID'),
    ...
    server.tool('taiga_update_sprint', 'Update sprint details. Must include version for optimistic concurrency.', {
        sprint_id: z.number().describe('Sprint (milestone) ID'),
        ...
        return client.patch(`/milestones/${sprint_id}`, {
            version,
            ...(name !== undefined && { name }),
