...
export function registerTaskTools(server, client, _cache) {
    server.tool('taiga_list_tasks', 'List tasks with flexible filters. Returns all results (pagination disabled).', {
    ...
    server.tool('taiga_get_task', 'Get task details by ID or reference number.', {
        ...
        if (task_id)
            return client.get(`/tasks/${task_id}`);
        if (ref && project_id)
            return client.get('/tasks/by_ref', { ref, project: project_id });
        throw new Error('Provide either task_id or both ref and project_id');
    }));
    server.tool('taiga_create_task', 'Create a new task, optionally linked to a user story and sprint.', {
    ...
    server.tool('taiga_update_task', 'Update a task. Must include version for optimistic concurrency.', {
        ...
        return client.patch(`/tasks/${task_id}`, rest);
    }));
    server.tool('taiga_get_task_history', 'Get the change history and comments for a task.', {
        task_id: z.number().describe('Task ID'),
    }, wrapTool(async ({ task_id }) => {
        return client.get(`/history/task/${task_id}`);
    }));
}
