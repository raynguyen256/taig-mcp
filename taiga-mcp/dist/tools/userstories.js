...
export function registerUserStoryTools(server, client, _cache) {
    server.tool('taiga_list_userstories', 'List user stories with flexible filters. Returns all results (pagination disabled).', {
        ...
        return client.getAll('/userstories', {
            project: project_id,
            milestone: milestone_id,
            ...rest,
        });
    }));
    server.tool('taiga_get_userstory', 'Get details for a user story by ID or by reference number + project.', {
        ...
        if (us_id)
            return client.get(`/userstories/${us_id}`);
        if (ref && project_id)
            return client.get('/userstories/by_ref', { ref, project: project_id });
        throw new Error('Provide either us_id or both ref and project_id');
    }));
    server.tool('taiga_create_userstory', 'Create a new user story in a project.', {
    ...
    server.tool('taiga_update_userstory', 'Update a user story. Must include version for optimistic concurrency.', {
        ...
        return client.patch(`/userstories/${us_id}`, {
            ...(milestone_id !== undefined && { milestone: milestone_id }),
            ...rest,
        });
    }));
    server.tool('taiga_move_userstory_to_sprint', 'Move one or more user stories to a sprint.', {
        project_id: z.number().describe('Project ID'),
    ...
    server.tool('taiga_get_userstory_history', 'Get the change history and comments for a user story.', {
        us_id: z.number().describe('User story ID'),
    }, wrapTool(async ({ us_id }) => {
        return client.get(`/history/userstory/${us_id}`);
    }));
}
