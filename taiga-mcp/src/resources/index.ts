...
export function registerResources(server: McpServer, client: TaigaClient) {
  // Static resource: all projects
  server.resource(
    'projects',
    'taiga://projects',
    { mimeType: 'application/json', description: 'List of all accessible Taiga projects' },
    async () => {
  ...
  server.resource(
    ...
    { mimeType: 'application/json', description: 'Project details by slug' },
    async (uri, { slug }) => {
  ...
  server.resource(
    ...
    { mimeType: 'application/json', description: 'All sprints for a project' },
    async (uri, { slug }) => {
      const project = await client.get<{ id: number }>('/projects/by_slug', { slug });
  ...
  server.resource(
    ...
    { mimeType: 'application/json', description: 'Backlog user stories (no sprint assigned)' },
    async (uri, { slug }) => {
      const project = await client.get<{ id: number }>('/projects/by_slug', { slug });
  ...
  server.resource(
    ...
    { mimeType: 'application/json', description: 'Team members of a project' },
    async (uri, { slug }) => {
      const project = await client.get<{ id: number }>('/projects/by_slug', { slug });
  ...
  server.resource(
    ...
    { mimeType: 'application/json', description: 'Summary statistics for a project' },
    async (uri, { slug }) => {
      const project = await client.get<{ id: number }>('/projects/by_slug', { slug });
