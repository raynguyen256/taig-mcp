...
import { TaigaEpic } from '../types/taiga.js';
import { wrapTool } from './utils.js';

export function registerEpicTools(server: McpServer, client: TaigaClient, _cache: Cache) {
  server.tool(
    'taiga_list_epics',
    ...
    wrapTool(async ({ project_id, ...rest }) => {
      return client.getAll<TaigaEpic>('/epics', { project: project_id, ...rest });
    }),
  );
  ...
  server.tool(
    ...
    wrapTool(async ({ epic_id }) => {
      return client.get<TaigaEpic>(`/epics/${epic_id}`);
    }),
  );

  server.tool(
    'taiga_get_epic_userstories',
    'Get all user stories related to an epic.',
    {
      epic_id: z.number().describe('Epic ID'),
    },
    wrapTool(async ({ epic_id }) => {
  ...
  server.tool(
    ...
    wrapTool(async ({ project_id, ...rest }) => {
      return client.post<TaigaEpic>('/epics', { project: project_id, ...rest });
    }),
  );
}
